...
@IonicPage()
@Component({
  selector: 'page-sub-category-add',
  templateUrl: 'sub-category-add.html',
})
export class SubCategoryAddPage {
  ...
  getItems(ev: any) {
    ...
    if (val && val.trim() != '') {
      this.productfilter = this.SubCategory.filter((item) => {
  ...
  createCategory() {
    ...
    if (this.productDTO.Id != "") {
      this.transactionProvider.UpdateProductSubCategory(this.productDTO,this.productDTO.Id,(result)=>{
        // console.log(result);
                if (result.IsSucess) {
      ...
      this.transactionProvider.CreateProductSubCategory(this.productDTO,(result)=>{
        console.log(result)
                if (result.IsSucess) {
              this.presentConfirm(result.Message);
            } else {
  ...
  loadallproduct() {
    ...
    this.transactionProvider.GetProductSubCategory((result)=>{
      console.log(result)
      this.SubCategory = result.data;
      this.productfilter = this.SubCategory;
    });
    this.transactionProvider.GetProductCategory((response)=>{
